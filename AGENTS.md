...
- **Engine plumbing vs game behaviour:** `IEngineSystem` (`MOBA.Engine.Core`) is for **engine-global plumbing** — drivers of cross-cutting external resources (input dispatch, asset loading, network transport, scene switching, render cadence) and scene-wide bookkeeping. Game-specific behaviour (champion abilities, minion waves, damage rules, projectile lifetimes, win conditions) belongs on a `Component` attached to the relevant actor, not as a new system that walks the scene. Litmus test: would this code make sense in a different game built on the same engine? If no, it isn't an engine system.
- **One top-level type per file.** Each `.cs` file declares exactly one public top-level type — `class`, `record`, `struct`, `enum`, or `interface` — named the same as the file. Tightly-coupled private helper types (a `record struct` only used inside one class, an in-file `enum` consumed solely by that class) may live nested *inside* the owning type; everything else gets its own file. Goal: when a type appears in a stack trace or a `using` directive, the file it lives in is unambiguous, and renaming the file = renaming the type.
