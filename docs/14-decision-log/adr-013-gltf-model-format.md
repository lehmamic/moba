...
2. **Engine-side API: `Model` + `LoadModel(name)`.** The asset surface the game sees is `MOBA.Engine.Graphics.Model` — one named asset, internally a `IReadOnlyList<ModelPart>` where each `ModelPart` is an `(IMesh, Material)` pair, plus convenience `Mesh`/`Material` accessors for the common single-part case. Loading is keyed by short name through the standard cache pattern: `assets.AddModelCache(backend, modelsRoot)` registers the cache, `assets.LoadModel("knight-garen")` returns the model. The cache resolves the file path internally (`{modelsRoot}/{name}.glb`). This mirrors the existing `LoadCubeMesh` / `LoadShader` / `LoadTexture` style — game code stays unaware that glTF is the on-disk format.

   **Ownership and disposal.** `Model` implements `IDisposable`: it owns the `IMesh` and any `ITexture` in its parts (both are constructed solely for this model at load time) and disposes them on shutdown. It does **not** own the `IShader` — shaders are shared resources from the shader cache, which disposes them on its own shutdown. `AssetCache.OnShutdown` runs Models' `Dispose` in LIFO order (model cache before texture/shader caches), so model-owned GPU handles are released before the underlying caches dispose their own pools.
...
5. **Shader registry — the shader cache itself.** `AssetManager`'s shader cache is keyed by short name (e.g. `"unlit_textured"`, `"phong_textured"`; future `"skinned_phong"`, `"toon"`, …); the on-disk pair lives at `{shadersRoot}/{name}.vert` + `{name}.frag`. The cache *is* the registry — adding a new shader is a matter of dropping the two GLSL files into `assets/shaders/`. The glTF loader reads the model material's `extras.shader` JSON field — if present, the string goes straight to `assets.LoadShader(name)` (unknown names fail at file-open time); if absent, the loader falls back to `"phong_textured"`. Asset files never carry GLSL source and never reference shader file paths.
...
- **Positive:**
  ...
  - The shader set stays project-owned. Adding a toon shader is just dropping `toon.vert` + `toon.frag` into `assets/shaders/` — no asset-file change required to *new* assets; existing assets opt in via `extras.shader`.
...
- **Future work this unblocks:** a toon shader (`toon.{vert,frag}` in `assets/shaders/`) + `material.extras.shader = "toon"` on one model is the smallest possible next step toward the Brawl-Stars-look without touching any code. Skeletal mesh support (M3) extends the loader to read joints/weights and `LogicalAnimations` without rewriting the format-decision side.